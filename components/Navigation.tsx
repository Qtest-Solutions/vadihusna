  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${scrolled
        ? 'py-2 bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-md'
        : 'py-4 bg-white/90 backdrop-blur-md border-b border-transparent'
        }`}
    >
      ...
      <div className={`transition-all duration-300 overflow-hidden ${scrolled ? 'h-0 opacity-0' : 'h-10 opacity-100'}`}>
        <div className="px-4 sm:px-6 lg:px-8 text-slate-600 text-sm flex justify-between items-center h-full border-b border-slate-100">
          <div className="flex items-center gap-5">
            <div className="flex items-center gap-2 hover:text-amber-600 transition-colors cursor-pointer">
              <Phone size={13} />
              <span className="text-xs font-medium tracking-wide">+91 9946100154</span>
            </div>
            <div className="hidden sm:flex items-center gap-2 hover:text-amber-600 transition-colors cursor-pointer">
              <Mail size={13} />
              <span className="text-xs font-medium tracking-wide">vhpseduc@gmail.com</span>
            </div>
          </div>
          <div className="hidden sm:block text-xs font-bold tracking-widest uppercase text-slate-400">
        ...
        <div className="flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center space-x-3 group">
            <img
              ...
              className={`object-contain transition-all duration-300 group-hover:scale-105 ${scrolled ? "h-14 sm:h-16" : "h-14 sm:h-16"
                }`}
            />
          </Link>
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-slate-800 px-4 py-2 text-[15px] hover:text-amber-600 font-semibold transition-colors relative group whitespace-nowrap"
              >
                {link.label}
            ...
            >
              <button className="text-slate-800 px-4 py-2 text-[15px] font-semibold flex items-center gap-1.5 hover:text-amber-600 transition-colors">
                Academics
                <ChevronDown
                  size={14}
      ...
      <div
        className={`lg:hidden absolute top-full left-0 w-full bg-white border-t border-slate-100 transition-all duration-300 ease-in-out shadow-2xl ${isOpen ? "max-h-[800px] opacity-100 py-4" : "max-h-0 opacity-0 overflow-hidden"
          }`}
      >
        <div className="px-4 space-y-2 container-max flex flex-col">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-slate-800 hover:text-amber-600 font-semibold py-3 px-4 rounded-lg hover:bg-slate-50 transition-colors text-lg"
          ...
          <div className="border-t border-slate-100 my-2 pt-2">
            <p className="text-xs font-bold tracking-widest text-slate-400 px-4 mb-2">ACADEMICS</p>
            {academicsDropdown.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="block text-slate-600 hover:text-amber-600 py-2.5 px-4 text-base rounded-lg hover:bg-slate-50 transition-colors pl-6"
                onClick={() => setIsOpen(false)}
              >
                {item.label}
              </Link>
